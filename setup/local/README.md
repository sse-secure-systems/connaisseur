...
In there, you will find a `helm` directory, which holds a `values.yaml` file, which needs to be configured. Add a new notary configuration with all appropriate values. The `selfsignedCert` field should be filled with the contents of the `harbor-ca.crt` from earlier.
...
For the `notaries[1].rootKeys[0].key` field, you need the public part of the Notary's `root` key. Its private component resides in your `~/.docker/trust/private` directory. With `openssl` you can get the public part form it, but you'll need to provide the passphrase you set, when generating the key:
...
Copy the contents of the public key into the `notaries[1].rootKeys[0].key` field of the `helm/values.yaml`. The create a new rule in the `policy` field of the `helm/values.yaml`, matching all images coming from the harbor registry, referencing harbor's notary.
