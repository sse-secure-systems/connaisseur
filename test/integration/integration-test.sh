...
## Backup charts/connaisseur/values.yaml
cp charts/connaisseur/values.yaml values.yaml.backup
cp charts/connaisseur/templates/deployment.yaml deployment.yaml.backup

DEFAULT_CTX=$(kubectl config current-context)

source test/integration/cleanup.sh
...
if [[ -n "${IMAGE+x}" && -n "${IMAGEPULLSECRET+x}" && "${GITHUB_WORKFLOW}" != "release" ]]; then
	yq '. *+ load("test/integration/var-img.yaml")' test/integration/ghcr-values.yaml >ghcr-tmp
	envsubst <ghcr-tmp >ghcr-values
	envsubst <test/integration/ghcr-values.yaml >ghcr-validator
	rm ghcr-tmp
else
	echo "" >ghcr-values
	echo "" >ghcr-validator
...
single_test() { # ID TXT TYP REF NS MSG RES
	...
	while :; do
		...
		if [[ "$3" == "deploy" ]]; then
			...
			envsubst <test/integration/workload-objects/$4.yaml | kubectl apply -f - >output.log 2>&1 || true
		else
...
multi_test() { # TEST_CASE: key in the `test_cases` dict in the cases.yaml
	...
	test_cases=$(yq e -o=json ".test_cases.$1" test/integration/cases.yaml)
...
workload_test() { # WORKLOAD_KIND
	...
	envsubst <test/integration/workload-objects/${KIND}.yaml | cat
	echo "::endgroup::"
...
complexity_test() { #
	...
	kubectl apply -f test/integration/deployments/complexity.yaml >output.log 2>&1 || true
...
load_test() { #
	...
	parallel --jobs 20 ./test/integration/cause_load.sh {1} :::: <(seq ${NUMBER_OF_INSTANCES}) >output.log 2>&1 || true
...
helm_install_release() { # NAMESPACE
	...
	helm repo add connaisseur https://sse-secure-systems.github.io/connaisseur/charts --force-update
	helm show values connaisseur/connaisseur | tee release.yaml >/dev/null # whyever >release.yaml doesn't work here # TODO: fix
	yq '. *+ load("ghcr-validator")' release.yaml | tee release_patched.yaml >/dev/null # whyever >release_patched.yaml doesn't work here # TODO: fix
...
helm_install_namespace() { # NAMESPACE
	...
	helm install connaisseur charts/connaisseur --atomic --create-namespace \
		--namespace ${1} >/dev/null || {
		echo -e "${FAILED}"
		exit 1
	}
...
helm_install_namespace_no_create() { # NAMESPACE
	...
	echo -n "Installing Connaisseur in namespace ${1}..."
	helm install connaisseur charts/connaisseur --atomic \
		--namespace ${1} >/dev/null || {
		echo -e "${FAILED}"
		exit 1
	}
...
helm_upgrade_namespace() { # NS
	echo -n 'Upgrading Connaisseur...'
	helm upgrade connaisseur charts/connaisseur -n ${1} --values "charts/connaisseur/values.yaml" --wait >/dev/null || {
		echo -e ${FAILED}
		exit 1
	}
	echo -e "${SUCCESS}"
...
update_values_minimal() {
	yq '. *+ load("ghcr-values")' -i charts/connaisseur/values.yaml
}

update_via_env_vars() {
	envsubst <test/integration/update.yaml >update
	yq '. *+ load("ghcr-values")' -i update
	yq eval-all --inplace 'select(fileIndex == 0) * select(fileIndex == 1)' charts/connaisseur/values.yaml update
	rm update
}
...
update_helm_for_workloads() {
	envsubst <test/integration/update-for-workloads.yaml >update
	yq '. *+ load("ghcr-values")' -i update
	yq eval-all --inplace 'select(fileIndex == 0) * select(fileIndex == 1)' charts/connaisseur/values.yaml update
	rm update
}
...
update_for_self_hosted_notary() {
	envsubst <test/integration/update-self-hosted-notary.yaml >update
	yq '. *+ load("ghcr-values")' -i update
	yq eval-all --inplace 'select(fileIndex == 0) * select(fileIndex == 1)' charts/connaisseur/values.yaml update
	rm update
...
enable_alerting() {
	envsubst <test/integration/update-alerting.yaml >update
	yq eval-all --inplace 'select(fileIndex == 0) * select(fileIndex == 1)' charts/connaisseur/values.yaml update
	rm update
}

render_notary_host() {
	envsubst <test/integration/update_deployment_notary_test.yaml >update_deployment_with_host_alias.yaml
...
regular_int_test() {
	...
	if [[ ${POD} == "" ]]; then
        echo -e "${FAILED}"
		echo "No pod found for edge case test."
	    EXIT="1"
	else
        ...
        if [[ "${DEPLOYED_SHA}" != 'fe542477b92fb84c38eda9c824f6566d5c2536ef30af9c47152fa8a5fadb58dd' ]]; then
	        echo -e "${FAILED}"
			echo "Expected image with SHA 'fe542477b92fb84c38eda9c824f6566d5c2536ef30af9c47152fa8a5fadb58dd' for Pod '${POD}'; got '${DEPLOYED_SHA}'"
	        EXIT="1"
        else
	        echo -e "${SUCCESS}"
        fi
...
### CERTIFICATE TEST ####################################
certificate_check() {
	DIFF=$(diff test/integration/tls.key <(kubectl get secrets -n connaisseur connaisseur-tls -o json | jq -r '.data."tls.key"' | base64 -d) || true)
	if [[ ${DIFF} != "" ]]; then
	...
	DIFF=$(diff test/integration/tls.cert <(kubectl get secrets -n connaisseur connaisseur-tls -o json | jq -r '.data."tls.crt"' | base64 -d) || true)
	if [[ ${DIFF} != "" ]]; then
...
"other-ns")
	...
	single_test "on" "Testing unsigned image..." "deploy" "securesystemsengineering/testimage:unsigned" "${NS}" "error during notaryv1 validation of image securesystemsengineering/testimage:unsigned: validated targets don't contain reference: no tag 'unsigned' found in targets" "INVALID"
	kubectl config use-context ${DEFAULT_CTX}
	;;
"deployment")
	update_via_env_vars
...
"configured-cert")
	...
	make_install
	certificate_int_test
	yq eval-all --inplace 'select(fileIndex == 0) * select(fileIndex == 1)' charts/connaisseur/values.yaml test/integration/update-cert.yaml
	make_upgrade
	certificate_check
	certificate_int_test
	;;
"redis-cert")
	source test/integration/redis-cert/redis_cert.sh
	redis_cert_test
	;;
"upgrade")
	...
	pre_config_int_test
	update_values_minimal
	debug_values "charts/connaisseur/values.yaml"
	helm_upgrade_namespace "connaisseur"
	# At this point in time, the old Connaisseur deployment might still be active
	# so wait a bit to ensure the new Connaisseur instances are tested
	sleep 10
