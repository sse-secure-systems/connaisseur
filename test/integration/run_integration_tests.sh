...
if [[ "${CLUSTER:-"minikube"}" == "minikube" ]]; then
  ...
  make docker > /dev/null
  echo "Done building docker image!"
  kind load docker-image $(yq e '.kubernetes.deployment.image.repository' charts/connaisseur/values.yaml):v$(yq e '.appVersion' charts/connaisseur/Chart.yaml)
...
for INTEGRATION_TEST in ${INTEGRATION_TEST_SUITE[@]}; do
  if [[ "${INTEGRATION_TESTS_TO_SKIP[@]}" =~ "${INTEGRATION_TEST}" ]]; then
    ...
    if [[ ${INTEGRATION_TEST} == 'self-hosted-notary' ]]; then
      ...
      docker run -d --name notary-signer -p 7899:7899 -v ./test/integration/notary_service_container/signer:/etc/docker/notary-signer/ --network ${NETWORK} notary:signer -config=/etc/docker/notary-signer/config.json
      ...
      docker run -d --name notary-server -p 4443:4443 --add-host notary.signer:${NOTARY_SIGNER_IP} -v ./test/integration/notary_service_container/server:/etc/docker/notary-server --network ${NETWORK} notary:server -config=/etc/docker/notary-server/config.json -logf=json
      ...
      docker build --build-arg "DIGEST=${DIGEST}" -f test/integration/Dockerfile.populate_notary . -t populate-notary
    ...
    ./test/integration/integration-test.sh ${INTEGRATION_TEST} |& tee ./tmp/${INTEGRATION_TEST}/test.log || true
    IS_INTEGRATION_TEST_RUNNING="false"

    echo "Done running integration test ${INTEGRATION_TEST}."
    mv connaisseur.log connaisseur.state connaisseur.yaml ./tmp/${INTEGRATION_TEST}/
